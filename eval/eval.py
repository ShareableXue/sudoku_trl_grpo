...
def calculate_score(data_path, result_path):
    ...
    for data in tqdm.tqdm(all_data, desc="Calculating scores"):
        ...
        total_scores = [1 if (r + c + b) == 1.0 else 0 for r, c, b in zip(row_rewards, col_rewards, block_rewards)]

        format_exclude = [1 if (r+c+b)==1.0 and f==0.6 else 0 for f, r, c, b in zip(format_rewards, row_rewards, col_rewards, block_rewards)]
    ...
    format_exclude_pct=format_exclude/len(all_data)/prompt_score_pct if prompt_score_pct>0 else 0.0
...
if __name__ == "__main__":
    import os
    os.makedirs("/root/project/sudoku-grpo/eval/score", exist_ok=True)
    
    data_path = "/root/project/sudoku-grpo/eval/data/grpo-7b-question-temperature0.jsonl"
    result_path = "/root/project/sudoku-grpo/eval/score/grpo-7b-results-temperature0.json"
